...
begin
  ...
  try
    ...
    AForm.RdgStatusAutorizacao.Enabled := False;
//    for I := 0 to AForm.RdgStatusAutorizacao.Items.Count - 1 do
//      AForm.RdgStatusAutorizacao.Controls[I].Enabled := False;
...
procedure TViewAutorizacaoCompra.fdQryTabelaAfterScroll(DataSet: TDataSet);
begin
  inherited;
  TbsAutorizacaoCancelado.TabVisible := (DtSrcTabela.DataSet.FieldByName('STATUS').AsInteger = STATUS_AUTORIZACAO_CAN);
end;

procedure TViewAutorizacaoCompra.fdQryTabelaBeforePost(DataSet: TDataSet);
begin
  inherited;
  with DtSrcTabela.DataSet do
  begin
